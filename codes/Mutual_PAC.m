...
% 3- Middle Frontal Cortex

load('presessionData_v1_normalizedLFPOverTime_EpochedData.mat')

if ~exist(fullfile(pwd,'MPAC'), 'dir')
    mkdir(fullfile(pwd,'MPAC'));
end
...
for c_a = 1:3 % c : channels
    for c_p = 1:3
     ...
     for k = 1:phase_length
        ...
        saveas(gcf,strcat(pwd,'\MPAC\event_',eve,channels(c_a),channels(c_p),'.png'))  
    ...
    %     pacfreq = [low_pacf, high_pacf];
        ...
        caxis([0, 1]); 
        colorbar;
        title(strcat('event:',eve,channels(c_a),channels(c_p),'Step:',int2str(cnt)));
        saveas(gcf,strcat(pwd,'\MPAC\event_',eve,channels(c_a),channels(c_p),'step_',int2str(cnt),'.png'))  
        ...
        plot(t,pac);
        set(gcf,'Position',get(0,'Screensize'));
        title(strcat('dynamic/event:',eve,channels(c_a),channels(c_p)));
        saveas(gcf,strcat(pwd,'\MPAC\dynamic_event_',eve,channels(c_a),channels(c_p),'.png'))
        close all;
    end
